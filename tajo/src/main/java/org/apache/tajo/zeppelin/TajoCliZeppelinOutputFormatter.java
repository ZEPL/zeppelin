...
/**
 * OutputFormatter for Zeppelin result.
 * @author hjkim
 */
public class TajoCliZeppelinOutputFormatter extends DefaultTajoCliOutputFormatter {
  ...
  public void init(TajoCli.TajoCliContext context) {
    super.init(context);
    maxResult = context.getInt("zeppelin.spark.maxResult",
        TajoSqlInterpreter.ZEPPELIN_SPARK_MAX_RESULT);
  }
  ...
  public void printResult(PrintWriter sout, InputStream sin, TableDesc tableDesc,
    ...
    if (res == null) {
      sout.println("<font color=red>" +
          getQuerySuccessMessage(tableDesc, responseTime, 0, "inserted", true) + "</font>");
      sout.flush();
      return;
    }
  ...
  private String getQuerySuccessMessage(TableDesc tableDesc, float responseTime,
                                        ...
                                        String postfix, boolean endOfTuple) {
    ...
    return "(" + displayRowNum + ", " +
        getResponseTimeReadable(responseTime) + ", " + volume + " " + postfix + ")";
  }
}
